...
const bottoken = "ODYzMzAzOTgxMDA5ODYyNjk3.YOk8fA.g7blGPGI-dKTnNy1FG5UbX9M3JA"
...
if (!fs.existsSync(dir)){
  ...
  fs.writeFile(`${dir}/warps.json`, "{}", function (err) {
    if (err) return console.log(err);
  });
  fs.writeFile(`${dir}/kits.json`, "{}", function (err) {
    if (err) return console.log(err);
  });
...
const warp = JSON.parse(fs.readFileSync(`${dir}/warps.json`, "utf8"))
const kits = JSON.parse(fs.readFileSync(`${dir}/kits.json`, "utf8"))
